...
app.post("/register", async (req, res) => {
    try {
        ...
        if (password != repeatPassword) {
            res.status(404).json({ message: "Both the Passwords should be same" });
        }
        else {
    ...
    } catch (err) {
        {
            ...
            if (errString.includes("email")) {
                res.status(404).json({ message: "E-mail is already in use" });
            }
            else if (errString.includes("name")) {
                res.status(404).json({ message: "Usename is already in use" });
            }
        }
    }
});
